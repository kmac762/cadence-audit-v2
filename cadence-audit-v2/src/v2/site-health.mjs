...
function examples(records,kind,max=8){return records.filter(r=>r.kind===kind).slice(0,max).map(r=>({url:r.url,finalUrl:r.finalUrl,status:r.status,redirectCount:r.redirectCount||0,sources:r.sources||[],linkOccurrences:r.linkOccurrences||[],sourceSitemap:r.sourceSitemap||null,error:r.error||null}));}
export async function inspectSiteHealth(facts,{mode='snapshot',maxTargets}={}){
 ...
 const sourcePages=[];
 const addSource=(url,hrefs,occurrences=[])=>{
   const source=normalize(url,entry);if(!source)return;
   const links=[];const seen=new Set();
   for(const item of occurrences||[]){const href=normalize(item?.href,source);if(!href||!sameSite(href,entry)||ASSET_RE.test(new URL(href).pathname))continue;const row={href,sourceUrl:source,anchor:String(item?.anchor||'').trim().slice(0,180),placement:item?.placement||'body'};const sig=`${href}|${row.anchor}|${row.placement}`;if(seen.has(sig))continue;seen.add(sig);links.push(row);}
   for(const value of hrefs||[]){const href=normalize(value,source);if(!href||!sameSite(href,entry)||ASSET_RE.test(new URL(href).pathname))continue;if(!links.some(x=>x.href===href))links.push({href,sourceUrl:source,anchor:'',placement:'unknown'});}
   if(links.length)sourcePages.push({source,links});
 };
 const entryHtml=facts?.contentAnalysis?.html||facts?.raw;
 if(facts?.contentAnalysis?.usable)addSource(entry,entryHtml?.internalHrefs||[],entryHtml?.internalLinkOccurrences||entryHtml?.internalLinkDetails||[]);
 const seenSources=new Set([normalize(entry,entry)]);
 for(const p of pages){if(!p?.usable)continue;const source=normalize(p.finalUrl||p.requestedUrl,entry);if(!source||seenSources.has(source))continue;seenSources.add(source);addSource(source,p.internalHrefs||p.primaryInternalHrefs||[],p.internalLinkOccurrences||p.internalLinkDetails||p.primaryInternalLinkDetails||[]);}

 const targetMap=new Map();
 for(const s of sourcePages)for(const link of s.links){const href=link.href;if(!targetMap.has(href))targetMap.set(href,{url:href,sources:[],linkOccurrences:[]});const t=targetMap.get(href);if(t.sources.length<4&&!t.sources.includes(s.source))t.sources.push(s.source);const sig=`${link.sourceUrl}|${link.anchor}|${link.placement}`;if(t.linkOccurrences.length<12&&!t.linkOccurrences.some(x=>`${x.sourceUrl}|${x.anchor}|${x.placement}`===sig))t.linkOccurrences.push({sourceUrl:link.sourceUrl,anchor:link.anchor,placement:link.placement});}
 ...
 for(const target of selected){
   ...
   results.push({...r,url:target.url,sources:target.sources,linkOccurrences:target.linkOccurrences||[],kind:classify(r)});
 }
...
export function siteHealthFindings(health){
 ...
 if(confirmed){
  const evidence=[...(health.examples.broken||[]),...(health.examples.serverErrors||[]),...(health.examples.loops||[]),...(health.examples.sitemapProblems||[])].slice(0,6).map(x=>({label:x.kind==='server-error'?'Server error':x.kind==='loop'?'Redirect loop':x.sourceSitemap?'Problem page from sitemap sample':'Broken internal destination',value:`${x.status?`HTTP ${x.status} - `:''}${x.url}`,url:x.linkOccurrences?.[0]?.sourceUrl||x.url}));
