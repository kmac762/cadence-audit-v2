import {mergeBrowserAssist} from './browser-assist.mjs';
...
export class JobManager {
 ...
 async addBrowserAssist(owner,id,captures){const j=this.get(owner,id);if(!j)throw Object.assign(new Error('Scan not found in this browser session.'),{status:404});if(!TERMINAL.has(j.state)||!j.report)throw Object.assign(new Error('Wait for the scan report before importing Browser Assist captures.'),{status:409});j.report=mergeBrowserAssist(j.report,Array.isArray(captures)?captures:[]);const valid=new Set((j.report.recommendations||[]).map(r=>r.id));j.reviews=Object.fromEntries(Object.entries(j.reviews||{}).filter(([id])=>valid.has(id)));await this.save(j);return{report:j.report,reviews:j.reviews};}
