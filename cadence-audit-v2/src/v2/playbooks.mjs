...
export const PLAYBOOKS={
  ...
  "template": {
    "title": "Fix Recurring On-Page Issues",
    "theme": "On-Page SEO",
...
export function recommendationTitle(f,key){
 ...
 if(key==='headings'){
  if(/multiple.*h1|multiple h1/.test(raw))return 'Multiple Header 1 Tags';
  if(/missing.*h1|no h1/.test(raw))return 'Missing Header 1 Tags';
  if(/empty.*h1/.test(raw))return 'Empty Header 1 Tags';
  if(/heading-level jump|heading level jump/.test(raw))return 'Heading Structure Issues';
  return 'Header 1 Tag Issues';
 }
 if(key==='titles'){
  if(/multiple.*title/.test(raw))return 'Multiple Title Tags';
  if(/missing.*title/.test(raw))return 'Missing Title Tags';
  if(/duplicate.*title/.test(raw))return 'Duplicate Title Tags';
  if(/placeholder.*title/.test(raw))return 'Placeholder Title Tags';
  if(/long.*title/.test(raw))return 'Long Title Tags';
  if(/short.*title/.test(raw))return 'Short Title Tags';
  return 'Improve Page Title Tags';
 }
 if(key==='metadata'){
  if(/multiple.*meta/.test(raw))return 'Multiple Meta Descriptions';
  if(/missing.*meta/.test(raw))return 'Missing Meta Descriptions';
  if(/duplicate.*meta/.test(raw))return 'Duplicate Meta Descriptions';
  if(/placeholder.*meta/.test(raw))return 'Placeholder Meta Descriptions';
  if(/long.*meta/.test(raw))return 'Long Meta Descriptions';
  if(/short.*meta/.test(raw))return 'Short Meta Descriptions';
  return 'Improve Meta Descriptions';
 }
 if(key==='links')return 'Implement Internal Link Strategy';
 if(key==='broken')return 'Fix Broken Internal Links';
 if(key==='indexing'){
  if(/noindex/.test(raw))return 'Review Noindex Directives';
  if(/canonical/.test(raw))return 'Fix Canonical Tag Issues';
  if(/sitemap/.test(raw))return 'Fix Sitemap & Indexing Issues';
  if(/redirect/.test(raw))return 'Fix Redirect Chains';
  return 'Resolve Indexing Issues';
 }
