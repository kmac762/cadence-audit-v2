...
function linkHealthCheck(linkHealth) {
  ...
  for (const result of linkHealth.results || []) {
    ...
    else if (result.accessRestricted || [401,403,407,429].includes(Number(result.status))) items.push({level:'review',issue:'Scanner access denied - link validity unknown',url:result.url,sourceUrl:linkHealth.sourceUrl||null,pageType:'linked URL',value:`HTTP ${result.status}; do not classify this as a broken link without independent verification`});
  ...
  const workers = Array.from({ length:Math.min(5, urls.length) }, async () => {
    while (true) {
      ...
      try {
        ...
        results[index] = { url, status:response.status, finalUrl:response.finalUrl, redirectCount:response.redirects?.length || 0,accessState:access.kind,accessRestricted:access.accessRestricted,requestAttempted:true };
      } catch (error) {
        results[index] = { url, status:null, finalUrl:url, redirectCount:0, requestAttempted:error.requestAttempted!==false&&!['ACCESS_PAUSED','ROBOTS_DISALLOWED'].includes(error.code), errorCode:error.code||'FETCH_ERROR', error:error instanceof Error ? error.message : 'Unable to validate link' };
      }
    }
  });
