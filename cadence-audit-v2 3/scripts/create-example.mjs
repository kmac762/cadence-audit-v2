...
const make=(key,id,obs,evidence)=>{const p=PLAYBOOKS[key],ai=AI_RELEVANCE[key]||{};return{id,playbook:key,title:p.title,theme:p.theme,observation:obs,why:p.why,searchEffect:p.searchEffect,aiLevel:ai.level||'',aiWhy:ai.why||'',aiEffect:ai.effect||'',aiSource:ai.source||'',aiTalk:ai.talk||'',solution:p.steps,success:p.success,verify:p.steps[0],caveat:p.caveat,effort:p.effort,priority:'Planned review',confidence:'Observed in sample output; interpretation needs review',talk:p.talk,source:p.source,evidence,scope:'Inspected sample',originalFinding:{title:'Synthetic '+key+' example',confidence:'manual-review'}};};
...
report.contentFreshness={enabled:true,checkedAt:report.scannedAt,articlesReviewed:8,counts:{likelyCurrent:5,worthRefreshing:2,strongRefreshCandidates:1,refreshCandidates:3,consolidationCandidates:1},candidates:[{url:'https://example.com/blog/guide-2022/',title:'Example Guide for 2022',published:'2022-05-12T00:00:00.000Z',modified:null,reasons:['page title references 2022','older year references appear in primary content','no newer structured modification date was detected'],level:'strong'}],consolidationCandidates:[{urlA:'https://example.com/blog/example-guide/',titleA:'Example Search Guide',urlB:'https://example.com/blog/example-search-tips/',titleB:'Example Search Tips Guide',similarity:.6,shared:['example','search','guide']}],limitations:['Synthetic freshness example. Age alone is never a finding.']};
report.coverage.contentFreshnessArticles=8;report.coverage.contentFreshnessCandidates=3;
